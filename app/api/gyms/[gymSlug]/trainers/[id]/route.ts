...
// GET /api/gyms/[gymSlug]/trainers/[id]
export async function GET(
  ...
  const profile = await prisma.profile.findFirst({
    where: { id, gymId: gym.id, role: 'trainer' },
  })
  if (!profile) {
    return NextResponse.json({ error: 'المدرب غير موجود' }, { status: 404 })
  }

  return NextResponse.json({ trainer: profile })
}

// PATCH /api/gyms/[gymSlug]/trainers/[id]
export async function PATCH(
  ...
  return NextResponse.json({ success: true, trainer: profile })
}

// DELETE /api/gyms/[gymSlug]/trainers/[id]
